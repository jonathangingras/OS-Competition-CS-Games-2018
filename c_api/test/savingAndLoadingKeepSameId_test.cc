...
TEST_F(SaveBuffer, savingAndLoadingKeepSameId) {
  ...
  size_t id = kiki_buffer_id(buffer);
  ...
  size_t rid = kiki_buffer_id(rbuffer);

  EXPECT_EQ(id, rid) << "buffers do not have same id";
}
